...
import { Home, Plus, Trash2, Building2 } from "lucide-react"
...
export default function RepresentadasPage() {
  ...
  async function excluirRepresentada(id: string, nome: string) {
    const confirmar = confirm(`Excluir representada "${nome}" ?`)
    if (!confirmar) return
    ...
    try {
      ...
      setRepresentadas((old) => old.filter((item) => item.id !== id))
    } catch {
      alert("Erro ao excluir representada")
    }
  }

  const representadasFiltradas = representadas.filter((rep) =>
    rep.nome.toLowerCase().includes(busca.toLowerCase())
  )
      ...
      <div className="sticky top-0 z-50 bg-white pb-4">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold">Representadas</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => router.push("/representadas/nova")}>
        ...
        {representadasFiltradas.map((rep, index) => (
          ...
          >
            ...
            <div
              ...
              onClick={() => router.push(`/representadas/${rep.id}`)}
            >
            ...
            <div className="col-span-1 flex justify-end">
              <Button
                variant="destructive"
                size="sm"
                onClick={() => excluirRepresentada(rep.id, rep.nome)}
              >
