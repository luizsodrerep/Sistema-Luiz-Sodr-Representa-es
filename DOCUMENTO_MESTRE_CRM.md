...
## 64. ATUALIZAÇÃO DE CONTINUIDADE — 08/09/2026

Este capítulo atualiza o estado operacional do CRM após os trabalhos
realizados entre os checkpoints `150836c`, `3e30b96` e `208be23`.

Quando houver conflito entre informações históricas dos capítulos
anteriores e este capítulo, prevalece o estado mais recente registrado
aqui.

### 64.1 Checkpoint funcional atual

Checkpoint de código protegido:
...
Push para `origin/main` concluído em 08/09/2026.

O checkpoint contém 16 arquivos alterados:
...
### 64.2 Ocorrência documental posterior ao checkpoint

Foi criado posteriormente o commit documental:

`a5c6f9b`

Mensagem:

`docs: atualiza estado operacional seguranca e financeiro do CRM`

Esse commit alterou somente:

`DOCUMENTO_MESTRE_CRM.md`

Ele reduziu acidentalmente o documento histórico e não deve ser usado
como base isolada de continuidade.

Nenhum arquivo funcional do CRM foi perdido ou alterado por esse commit.

A correção adotada foi restaurar o Documento Mestre completo do
checkpoint `208be23` e acrescentar este capítulo de atualização.

Não reescrever histórico Git e não fazer force push por causa dessa
ocorrência.

### 64.3 Regra de não duplicação

Existe somente UM sistema CRM oficial.

Não criar como solução de segurança:

- outro projeto;
- outra pasta do CRM;
- outro repositório;
- clone paralelo;
- branch denominada backup;
- segunda instalação do sistema.

O histórico Git/GitHub deve proteger versões dentro do mesmo projeto.

Uma cópia de segurança de dados PostgreSQL é conceitualmente diferente
de duplicar o sistema, mas qualquer procedimento desse tipo deverá ser
feito somente quando explicitamente aprovado e de forma controlada.

### 64.4 Arquivo local fora do Git

Continua propositalmente não versionado:

`public/foto sistema minha area paula.jpeg`

Estado esperado no Git quando o código e a documentação estiverem
limpos:

`?? "public/foto sistema minha area paula.jpeg"`

Não executar `git add .` enquanto não houver decisão explícita sobre
esse arquivo.

### 64.5 Segurança das APIs — lote atual

Foram reforçadas APIs que anteriormente possuíam riscos de exposição
global ou ausência de isolamento adequado.

Importação de Clientes:

`app/api/clientes/importar/route.ts`

Agora exige sessão, permissão, escritório, arquivo XLSX válido, limite
de tamanho e criação vinculada ao escritório/usuário.

Exportação de Clientes:

`app/api/clientes/exportar/route.ts`

Agora exige sessão, permissão, escritório e respeita o escopo do
Preposto.
...
Agora possui GET e POST reais, isolamento por escritório, validação de
titularidade e cadastro operacional de contas.

Empresas do escritório:

`app/api/empresas-escritorio/route.ts`

Agora exige sessão, permissão e `escritorioId`.

APIs aninhadas de Representadas também foram reforçadas:

- contratos;
- contrato por ID;
- regras comerciais;
- regra comercial por ID;
- contas de recebimento.

Toda entidade aninhada deve pertencer ao mesmo escritório da sessão.

### 64.6 Confidencialidade e Preposto

Permanece pendente uma matriz granular definitiva de permissões.

Um Preposto poderá precisar visualizar condições operacionais como:
...
Isso não significa autorização automática para visualizar:

- percentual bruto de comissão recebido pelo escritório;
- documentos contratuais confidenciais;
- contas bancárias;
- dados financeiros;
- regras internas de remuneração;
- fechamento de comissão.

A segurança futura deverá possuir:

1. perfil base;
2. permissão granular por usuário;
3. proteção de menu;
4. proteção de página;
5. proteção de API;
6. filtragem/ocultação de campos confidenciais no backend.

### 64.7 Agenda

`app/agenda/page.tsx`

Os dados demonstrativos principais foram removidos.

A Agenda utiliza atualmente:

`/api/meu-assistente-pessoal`

Possui visões mensal, semanal e diária, filtros, marcações, listas,
próximos compromissos, itens sem data, CSV e acesso a Nova Interação.

Pendência estrutural:

ainda não existe entidade persistente própria para tarefas e
compromissos administrativos livres.

Decisão:

uma Tarefa/Compromisso NÃO deve ser armazenada artificialmente como
Interação.

Futuramente deverá existir entidade própria, com possibilidade opcional:

`Registrar também como Interação`

### 64.8 Interações — Análise

`app/interacoes-ai/page.tsx`

Foi conectado a dados reais de:

`/api/interacoes`

Possui indicadores, filtros, detalhes, CSV e atualização automática.
...
Estrutura funcional antiga foi preservada.

Métricas e dados fictícios principais foram removidos.

Integrações reais com plataformas sociais ainda estão pendentes.

Nunca apresentar lead, alcance ou engajamento como real sem fonte real.

### 64.10 Relatórios

`app/relatorios/page.tsx`

Foi conectado principalmente a:

- `/api/vendas`;
- `/api/clientes`.

Possui períodos, filtros, categorias, Representadas, comissão e
exportação CSV com dados reais disponíveis.

Vendas canceladas não devem inflar indicadores principais.

Não criar ranking fictício de produtos quando não houver base real de
itens.

### 64.11 Financeiro — prioridade operacional atual

Financeiro tornou-se a principal frente operacional em 08/09/2026.

Objetivo:

permitir cadastrar e acompanhar a posição financeira real do escritório,
incluindo:
...
### 64.12 Financeiro — conta e saldo inicial

O Prisma existente já possui estruturas utilizáveis:

- `ContaBancaria`;
- `Financeiro`;
- vínculo `contaBancariaId`.

Não foi necessária alteração de schema para a primeira fase.

Cada conta pode possuir seu próprio saldo inicial.
...
Saldo inicial NÃO é despesa.

Saldo inicial NÃO é origem externa.

Foram realizados testes reais com uma conta de saldo negativo e outra
com saldo positivo.

Os valores privados não devem ser registrados neste documento
versionado.

O saldo consolidado entre as contas foi calculado corretamente.

### 64.13 Financeiro — correção de data

Foi identificado erro de fuso horário em data financeira.

Uma data digitada como:
...
interpretação de `YYYY-MM-DD` como UTC seguida de conversão para fuso
local.

A tela Financeiro foi corrigida para preservar a data operacional
informada.

Teste visual posterior confirmou a data correta.

### 64.14 Financeiro — transferência entre contas

A função:

`Transferência entre contas`

foi implementada no backend e frontend.

Regra contábil:

uma transferência entre contas próprias NÃO é receita nem despesa do
escritório.
...
Conta origem:  - R$ X
Conta destino: + R$ X
Consolidado:     sem alteração

O backend registra duas pontas dentro de uma transação Prisma:

- Saída na conta de origem;
- Entrada na conta de destino.

As duas pontas possuem identificador técnico comum.

Cancelamento deve atingir as duas pontas.

Exclusão definitiva deve atingir as duas pontas.

Descrição visível:

`Transferência entre contas`

Não é necessário expor motivo particular da transferência.

Estado:

IMPLEMENTADA E VALIDADA POR TYPESCRIPT.

Ainda pendente:

PRIMEIRO TESTE FUNCIONAL REAL COMPLETO.

O notebook desligou inesperadamente antes da conclusão desse teste.

### 64.15 Financeiro — extrato por conta

Foi implementado acesso:

`Ver extrato`

nos cards das contas.

Esse extrato representa os MOVIMENTOS REGISTRADOS NO CRM.

Ele ainda NÃO representa sincronização automática com o extrato oficial
do banco.

Uma transferência deve aparecer como movimentação entre contas próprias.

Integração bancária/Open Finance é tema futuro e exige avaliação
específica de segurança.

### 64.16 Cartões, dívidas e renegociações

Na estrutura atual:

cartão de crédito é tratado como obrigação/fatura e não como conta
bancária.

Dívidas e acordos podem ser lançados como saídas pendentes.

Parcelamentos podem ser criados pela API Financeiro.

Antes de carga financeira massiva devem ser concluídos testes de:
...
### 64.17 Limite bancário

Limite/cheque especial NÃO deve ser somado ao saldo bancário como se
fosse dinheiro disponível próprio.

Estrutura conceitual futura deve separar:
...
### 64.18 Prisma — lote estrutural futuro

NÃO alterar Prisma agora sem diagnóstico.

Um único lote estrutural futuro deverá avaliar conjuntamente:

- Compromissos/Tarefas;
- Cadastro 360 do Preposto/Vendedor;
- permissões granulares por usuário;
- remuneração versionada;
- dados bancários protegidos;
- comissões devidas/pagas ao Preposto;
- múltiplos contatos de Representadas;
- eventual estrutura própria de cartões/limites;
- regras comerciais antigas versus versionadas.

Também deverá avaliar:

- dinheiro armazenado como `Float`;
- eventual migração para `Decimal` ou centavos inteiros;
- campos de tenant ainda opcionais;
- unicidade de `Cliente.codigo`.

Nenhuma dessas mudanças foi autorizada ou executada neste lote.

### 64.19 Regra sobre banco e Prisma

Antes de qualquer alteração estrutural, executar somente diagnóstico
não destrutivo:
...
Se Prisma sugerir perda de dados ou reset:

CANCELAR e analisar antes.

### 64.20 Incidente do notebook em 08/09/2026

O notebook de Luiz desligou inesperadamente durante a operação.

A causa ainda NÃO foi diagnosticada.

Não atribuir automaticamente o desligamento ao CRM sem evidência.

Após reiniciar:

- arquivos locais permaneceram disponíveis;
- TypeScript passou;
- `git diff --check` passou;
- checkpoint `208be23` foi criado;
- checkpoint foi enviado ao GitHub.

O incidente motivou uma validação geral antes da continuação funcional.

### 64.21 Servidor local e acesso de Paula

O CRM é utilizado por Luiz e Paula na rede local.

Comando utilizado para disponibilização em LAN:

`npm run dev -- -H 0.0.0.0`

Último IP confirmado antes do desligamento inesperado:

`192.168.0.57`

Como endereço IP de rede local pode mudar após reinicialização,
reconfirmar com `ipconfig` antes de assumir que esse IP continua válido.

Quando o IP for confirmado, Luiz e Paula devem preferir o mesmo host.

Evitar utilizar simultaneamente:
...
porque cookies de autenticação são vinculados ao host.

Já ocorreu falha operacional por essa diferença de host.

### 64.22 Regra crítica sobre build

NÃO executar:

`npm run build`

enquanto Paula estiver utilizando `npm run dev`.

Já ocorreu corrupção/instabilidade da pasta `.next` ao misturar build e
servidor dev no mesmo ambiente.

Antes de build verificar se existe processo `next dev`.

Build completo deve ser executado somente em janela segura.

### 64.23 Auditoria não estrutural ainda pendente

Antes do lote Prisma, continuar revisão de:

- Usuários com dados demonstrativos;
- Configurações com falsa persistência;
...
- SalesComparison;
- Mapa;
- calendário Financeiro antigo;
- ShareButtons;
- Templates;
- consentimento de e-mail de Clientes;
- RBAC/sidebar;
- login/rate limiting;
- vulnerabilidades npm;
- identidade visual;
- quality gates do Next.js.

Regra:

não apagar estrutura útil do frontend.

Remover dados falsos, não funcionalidades legítimas.

### 64.24 Próxima conversa — ordem exata

A próxima conversa deve começar pela verificação técnica, sem refazer os
16 arquivos do checkpoint `208be23`.

Ordem:

1. confirmar Git/GitHub;
2. confirmar working tree;
3. validar Prisma;
4. confirmar estado das migrations;
5. confirmar conexão e integridade aparente do PostgreSQL;
6. verificar processo `next dev`;
7. somente executar build se Paula não estiver trabalhando;
8. validar TypeScript;
9. validar Git;
10. retomar Financeiro.

Não solicitar novamente dezenas de arquivos já auditados.

Usar GitHub e este Documento Mestre.

### 64.25 Próximo passo funcional exato

Após a validação geral:

RETOMAR FINANCEIRO — TRANSFERÊNCIA ENTRE CONTAS.

Não reescrever Financeiro.

Primeiro:

- abrir Financeiro;
- confirmar botão `Transferência entre contas`;
- confirmar modal;
- escolher conta de origem;
- escolher conta de destino;
- informar valor;
- conferir data;
- parar na confirmação;
- verificar que o consolidado não muda;
- registrar somente após conferência;
- verificar extrato das duas contas;
- verificar saldo individual;
- verificar saldo consolidado.

O CRM NÃO realiza PIX bancário.

O usuário executa a movimentação no banco.

O CRM registra o fato financeiro.

### 64.26 Manual Operacional

Durante os testes, o assistente deve ensinar como utilizar corretamente
os formulários e fluxos.

Exemplos:

dinheiro já existente na abertura do controle
= Saldo inicial

dinheiro recebido depois
= Entrada

dinheiro pago depois
= Saída

dinheiro transferido entre contas próprias
= Transferência entre contas
...
### 64.27 Mensagem recomendada para abrir nova conversa

"Estamos continuando o projeto Sistema CRM Luiz Sodré Representações.

Leia integralmente o DOCUMENTO_MESTRE_CRM.md, principalmente o capítulo
64, e consulte o GitHub quando necessário.

O checkpoint funcional protegido de código é `208be23`.

Não quero refazer diagnósticos nem solicitar novamente os 16 arquivos
já validados nesse checkpoint.

Houve um desligamento inesperado do notebook em 08/09/2026 e decidimos
primeiro verificar de forma não destrutiva Git/GitHub, Prisma e
PostgreSQL antes de continuar.

Não execute `migrate reset`, `db push`, migration nova, alteração de
schema, segundo projeto, branch de backup ou cópia paralela do CRM sem
diagnóstico e minha aprovação.

Não execute `npm run build` se existir `next dev` ativo ou Paula estiver
utilizando o CRM.

Depois da validação geral, retome exatamente o Financeiro em
Transferência entre contas, já implementada em backend e frontend mas
ainda pendente do primeiro teste funcional real completo.

Preserve
`public/foto sistema minha area paula.jpeg`
fora do Git.

Não apague funcionalidades legítimas do frontend apenas porque alguma
integração ainda não existe.

Para mudanças manuais de arquivo, continue priorizando substituição
